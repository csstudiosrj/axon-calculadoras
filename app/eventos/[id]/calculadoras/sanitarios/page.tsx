...
function CalculadoraSanitariosContent() {
  ...
  const { canCalculate, playAd, decrementUse } = useMonetization();
  ...
  const handleCalcular = async (e: React.FormEvent) => {
    ...
    // Anuncio de 15s antes de mostrar o resultado
    await playAd("post");
    ...
    try {
      ...
      // decrementUse atualiza uses_count E extra_credits corretamente
      await decrementUse(evento.id, evento.uses_count);

      // Atualiza estado local para refletir novo uses_count
      setEvento((prev) => prev ? { ...prev, uses_count: prev.uses_count + 1 } : prev);

    } catch (error) {
      console.error("Erro na logica de calculo:", error);
      alert("Ocorreu um erro ao processar as normas tecnicas.");
    }
  ...
  return (
    <div className="max-w-6xl mx-auto mt-6 pb-12 px-4">
      ...
      <div className="flex items-center gap-5 mb-8 bg-zinc-900/50 border border-zinc-800/80 p-6 rounded-3xl backdrop-blur-sm">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Dimensionamento de Sanitarios</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-xl">
          <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            ...
            Parametros Tecnicos do Evento
          </h2>

          <form onSubmit={handleCalcular} className="space-y-6">

            <div className="bg-zinc-950/50 p-5 rounded-2xl border border-zinc-800/50">
              <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">Dados de Publico</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-zinc-500 mb-1">Publico Base (PAX)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Duracao (Horas)</label>
                  <select
                    value={duracao}
                  ...
                  >
                    <option value="4">Ate 4 horas</option>
            ...
            <div className="bg-zinc-950/50 p-5 rounded-2xl border border-zinc-800/50">
              <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">Caracteristicas</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Proporcao de Genero</label>
                  <select
                    value={proporcao}
            ...
            >
              {isCalculating ? "Processando Normas Tecnicas..." : "Processar Dimensionamento"}
            </button>
          </form>
        </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-xl flex-grow flex flex-col relative overflow-hidden">
            ...
            {!resultado ? (
              <div className="flex-grow flex flex-col items-center justify-center text-zinc-600 text-center relative z-10">
                ...
                <p className="text-sm">Preencha os parametros e processe para visualizar o memorial de calculo.</p>
              </div>
            ) : (
              <div className="space-y-3 relative z-10">
                ...
                <div className="flex justify-between items-center p-4 bg-zinc-950/80 rounded-xl border border-zinc-800/80">
                  <span className="text-zinc-300 font-medium">Mictorios</span>
                ...
                <div className="flex justify-between items-center p-4 bg-zinc-950/80 rounded-xl border border-zinc-800/80">
                  <span className="text-zinc-300 font-medium">Pias / Lavatorios</span>
                ...
                {resultado.alerts && resultado.alerts.length > 0 && (
                  <div className="mt-6 p-5 bg-red-950/30 border border-red-900/50 rounded-xl">
                    <h4 className="text-red-400 font-bold mb-3 text-sm uppercase tracking-wider">
                      Alertas Normativos
                    </h4>
