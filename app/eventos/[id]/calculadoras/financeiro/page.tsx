...
// @ts-ignore
import { calcFinanceiro } from "@/lib/calculators/calculations";
...
function CalculadoraContent() {
  ...
  const handleCalcular = async (e: React.FormEvent) => {
    ...
    try {
      const res = (calcFinanceiro as any)(calcParams as any);
      setResultado(res);
      setSalvando(true);
  ...
  return (
    <div className="max-w-6xl mx-auto mt-6 pb-12 px-4">
      ...
      <div className="flex items-center gap-5 mb-8 bg-zinc-900/50 border border-zinc-800/80 p-6 rounded-3xl">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Financeiro & Lucro</h1>
          <p className="text-sm mt-1" style={{ color: "#3ecf8e" }}>DRE Simplificado · Break-even · ROI</p>
        </div>
      </div>
