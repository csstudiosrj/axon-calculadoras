...
// @ts-ignore
import { calcResiduos } from "@/lib/calculators/calculations";
...
function CalculadoraContent() {
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-[50vh]"><div className="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin" style={{ borderColor: "#4CAF50", borderTopColor: "transparent" }} /></div>;
  ...
  return (
    <div className="max-w-6xl mx-auto mt-6 pb-12 px-4">
      ...
      <div className="flex items-center gap-5 mb-8 bg-zinc-900/50 border border-zinc-800/80 p-6 rounded-3xl">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Resíduos Sólidos</h1>
          <p className="text-sm mt-1" style={{ color: "#4CAF50" }}>NBR 16366 · PNRS Lei 12.305/2010</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 bg-zinc-900 border border-zinc-800 rounded-3xl p-8">
          <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
            <span className="w-1.5 h-6 rounded-full" style={{ backgroundColor: "#4CAF50" }} />
          ...
          <form onSubmit={handleCalcular} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="area" type="number" placeholder="ex: 5000" min={1} className="w-full px-4 py-3 bg-zinc-950 border border-zinc-700 rounded-xl text-white focus:outline-none transition-colors" style={{ outlineColor: "#4CAF50" }} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
            ...
            <button type="submit" disabled={isCalculating} className="w-full text-white font-bold text-base py-4 rounded-xl transition-all disabled:opacity-50 mt-2" style={{ background: `linear-gradient(135deg, #4CAF50, #4CAF5099)` }}>
...
export default function Page() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-[50vh]"><div className="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin" style={{ borderColor: "#4CAF50", borderTopColor: "transparent" }} /></div>}>
      <CalculadoraContent />
    </Suspense>
  );
}
